...
// IMPORTANT: __dirname is unreliable here because Next.js bundles each API
// route separately, which relocates this file's compiled output — so
// __dirname pointed at the wrong folder entirely at runtime (confirmed via
// the debug endpoint). process.cwd() reliably resolves to the project root
// in Vercel's serverless functions, so we build the path from there instead.
const CUSTOM_PAGE2_PATH = path.join(process.cwd(), "lib", "indigenous.jpg");
...
const FIELDS = {
  name: { x: 0.195, yTop: 0.3379 },
  surname: { x: 0.2137, yTop: 0.3722 },
  dob: { x: 0.2661, yTop: 0.4065 },
  clan: { x: 0.1774, yTop: 0.4380 },
  address: { x: 0.2056, yTop: 0.4705 },
  idNo: { x: 0.4043, yTop: 0.6386 },
  gender: { x: 0.3935, yTop: 0.6740 },
  issuedOn: { x: 0.7409, yTop: 0.6004 },
};

// Photo frame box, as fractions of the full page (top-left origin), also
// measured pixel-precise against the same background asset. Slightly inset
// from the outer border lines (particularly top-right, which has a small
// angled corner cut in the template) so the photo never overflows past it.
const PHOTO_BOX = { left: 0.0907, right: 0.3462, top: 0.5707, bottom: 0.7529 };
...
function getCustomPage2Diagnostic() {
  const result = {
    checkedPath: CUSTOM_PAGE2_PATH,
    exists: false,
    sizeBytes: null,
    error: null,
  };
  try {
    result.exists = fs.existsSync(CUSTOM_PAGE2_PATH);
    if (result.exists) {
      result.sizeBytes = fs.statSync(CUSTOM_PAGE2_PATH).size;
    }
  } catch (err) {
...
module.exports = { generateCertificatePdf, getCustomPage2Diagnostic };
